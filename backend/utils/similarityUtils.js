// Backend utility functions for smart similarity matching
...
const calculateSimilarity = (product1, product2) => {
  ...
  const categoryMatch = product1.category?.toString() === product2.category?.toString();
...
const findSimilarProducts = (currentProduct, allProducts, limit = 8) => {
  if (!currentProduct || !allProducts || allProducts.length === 0) return [];
  
  // Filter out the current product
  const otherProducts = allProducts.filter(p => p._id.toString() !== currentProduct._id.toString());
...
const findSimilarMedicines = (currentMedicine, allMedicines, limit = 8) => {
  if (!currentMedicine || !allMedicines || allMedicines.length === 0) return [];
  
  // Filter out the current medicine
  const otherMedicines = allMedicines.filter(m => m._id.toString() !== currentMedicine._id.toString());
