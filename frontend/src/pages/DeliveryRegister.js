...
export default function DeliveryRegister() {
  ...
  useEffect(() => {
    ...
    fetch(joinUrl(API_BASE, `/auth/verify-invite-token?role=deliveryBoy&token=${token}`))
      .then(res => res.json())
      .then(data => {
        if (data.valid) setTokenValid(true);
        else setError(data.message || 'Invalid or expired invite token.');
      })
  ...
  const handleSubmit = async e => {
    ...
    setSuccess('');
    setLoading(true);
    // Prepare registration data as per backend schema
    ...
    if (res.token) {
      ...
      navigate('/login', { state: { success: 'Registration complete. Wait for admin approval.' } });
  ...
  if (!tokenValid) return <Typography align="center" sx={{ mt: 8 }}>Verifying invite token...</Typography>;
      ...
      <Fade in timeout={800}>
        ...
        >
          ...
          <Typography variant="subtitle1" textAlign="center" color="text.secondary" mb={2}>
            Register for MediCare (Delivery Boy)
          </Typography>
          <form
          ...
          >
            ...
            <TextField
              ...
              sx={commonTextFieldStyle}
            />
            <FormControl fullWidth required sx={commonTextFieldStyle}>
