          ...
          <div style={{ minWidth: 220, maxWidth: 260, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10, alignItems: 'center', marginBottom: 8 }}>
              {images.map((img, idx) => (
                ...
                >
                  <img src={`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}${img}`} alt={medicine.name} style={{ width: 48, height: 48, objectFit: 'contain', borderRadius: 6 }} />
                </div>
              ))}
            </div>
            {/* Main image card */}
            <div style={{ background: '#f8fbff', borderRadius: 14, boxShadow: '0 2px 8px rgba(25,118,210,0.07)', padding: 18, textAlign: 'center', minWidth: 180, minHeight: 220, position: 'relative' }}>
              ...
              <img src={`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}${images[selectedImage]}`} alt={medicine.name} style={{ width: 220, height: 160, objectFit: 'contain', margin: '18px 0 10px' }} />
            </div>
          </div>
