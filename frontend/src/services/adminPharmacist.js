const API = process.env.REACT_APP_API_URL || 'https://medicare-ydw4.onrender.com/api/admin';
...
export async function getPharmacistSales() {
  const res = await fetch(`${API}/pharmacist-sales`, {
...
export async function getPharmacistCustomers() {
  const res = await fetch(`${API}/pharmacist-customers`, {
    headers: { 'Authorization': `Bearer ${getToken()}` }
  });
...
export async function getPharmacistSuppliers() {
  const res = await fetch(`${API}/pharmacist-suppliers`, {
    headers: { 'Authorization': `Bearer ${getToken()}` }
  });
...
export async function getPharmacistAnalytics() {
  const res = await fetch(`${API}/pharmacist-analytics`, {
    headers: { 'Authorization': `Bearer ${getToken()}` }
  });
